...
String noteStr = "3+43+13+40 41+43+25+20 45 20 41+20 45 40 43 20 46 17 40 47 46 25 23+25+26+24+45+40 43+11+42+47 10 4";

void setup()
{
  Serial.begin(115200);
  noteStr += "1+10 45 40 23 20 46 17 40 47 46 25 23+25+26+14+45+40 43+11+42+47 10 40 15+44+44+43+10 45 46 41+40 46 41+42+4";
  pinMode(A3,INPUT);
  pinMode(A1,INPUT);
  DIDR0 = 0x3F;   //disable the digital input buffers on the analog pins to save a bit of power and reduce noise.

  edgar.begin(CHA); //start the synth with output on Pin 11
//
...
int index = 0;
int c = 57;
// int notes[13] =    {3,2, 1,  3,  2,  1, 8,6, 8,5,3,1,2};
// float delays[13] = {2,0.5,0.5,0.5,0.5,1,1,0.5,1.5,2,1,1,4};
...
void setPianoNote(int track, int n,float t){
    edgar.setupVoice(track, analogRead(A3)/75, n, 0, 120, 64);
    edgar.setTime(track,fminf(12,fmaxf(1,analogRead(A1)/400*t)));
    edgar.trigger(track);
}
float parseStrNote(int noteIndex){
  String pitch = "";
  pitch+=noteStr.charAt(noteIndex*3);
  char level = noteStr.charAt(noteIndex*3+1);
  String t = "";
  t+=noteStr.charAt(noteIndex*3+2);
  int note = level=='+'?ho[pitch.toInt()]:(level=='-'?lo[pitch.toInt()]:mo[pitch.toInt()]);
  float during = 1.0/t.toInt();
  setPianoNote(0,note,during);
  return during*500;
}
int buffer[5] = {0,0,0,0,0};
void parseNote(int track,int note,int during){
  setPianoNote(track,note,min(10,max(during,1)));
}
void loop()
{
  if(Serial.available()){
    ...
    index++;
    if(c==0xff){
      parseNote(buffer[0]%4,buffer[1],buffer[2]);
      index = 0;
    }
  }
  // if(millis() - triggerTime >= parseNote(index))  {
  //   // setPianoNote(notes[index],delays[index]);
  //   // edgar.setMod(0,voicePrefs[Current][index][4]);
  //   index++;
  //   if(index>=noteStr.length()/3){
  //     index = 0;
  //   }
  //   triggerTime=millis();
  // }
